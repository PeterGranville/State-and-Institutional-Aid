...
loadDatalab1 <- function(
  ...
  if(inStateSelector==TRUE){
    workingDir <- paste("/Users/peter_granville/Net Price Equity/NPSAS Data (In-State)/", sectionName, sep="")
  }else{
...
fileScan <- function(fileName, folderName, subtableSelect){
  ...
  if(inStateSelector==TRUE){
    workingDir <- paste("/Users/peter_granville/Net Price Equity/NPSAS Data (In-State)/", folderName, sep="")
  }else{
...
folderScan <- function(folderName0, subtableSelect0){
  ...
  if(inStateSelector==TRUE){
    workingDir <- paste("/Users/peter_granville/Net Price Equity/NPSAS Data (In-State)/", folderName0, sep="")
  }else{
...
if(inStateSelector==TRUE){
  write.csv(AMP.INSTSTAT.2Y, "AMP-INSTSTAT-2Y (In-State).csv", row.names=FALSE)
  write.csv(AMP.INSTSTAT.4Y, "AMP-INSTSTAT-4Y (In-State).csv", row.names=FALSE)
  write.csv(AMP.SECTOR3, "AMP-SECTOR3 (In-State).csv", row.names=FALSE)
  write.csv(DIST.SECTOR3, "DIST-SECTOR3 (In-State).csv", row.names=FALSE)
  write.csv(axisBounds, "Axis-Bounds (In-State).csv", row.names=FALSE)
}else{
  write.csv(AMP.INSTSTAT.2Y, "AMP-INSTSTAT-2Y (Overall).csv", row.names=FALSE)
  write.csv(AMP.INSTSTAT.4Y, "AMP-INSTSTAT-4Y (Overall).csv", row.names=FALSE)
  write.csv(AMP.SECTOR3, "AMP-SECTOR3 (Overall).csv", row.names=FALSE)
  write.csv(DIST.SECTOR3, "DIST-SECTOR3 (Overall).csv", row.names=FALSE)
  write.csv(axisBounds, "Axis-Bounds (Overall).csv", row.names=FALSE) 
}

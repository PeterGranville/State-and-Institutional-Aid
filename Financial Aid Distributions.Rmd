...
chartFn1 <- function(targetName, rowName, measureName, subtable, sector3Val){
  ...
  plot1 <- ggplot(
    data=tempDF, mapping=aes(x=`Target value`, y=`Row value`)
  ...
  if(measureName %in% c("Median", "Average")){
    plot1 <- plot1 + scale_x_continuous(
      labels=dollar_format(accuracy=1, limits=c(0, 1))
    ) 
  }
...
chartFn2 <- function(distributionName, rowName){
  ...
  # Title function not needed 
  
  return(print(paste(distributionName, rowName, sep=" | ")))
